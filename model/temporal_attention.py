...
class TemporalAttentionLayer(torch.nn.Module):
  # 于 PyTorch 的时序注意力层，用于在动态图中融合节点自身特征、邻居特征和时间信息，生成包含时序依赖的节点嵌入。核心思想是通过注意力机制学习邻居对当前节点的影响权重，同时考虑交互发生的时间因素。
  """
  ...
  def forward(self, src_node_features, src_time_features, neighbors_features,
    ...
    # 输出 attn_output 是融合邻居信息的嵌入（形状 [1, batch_size, query_dim]），attn_output_weights 是每个邻居的注意力权重（形状 [batch_size, 1, n_neighbors]）。
    ...
    # # 合并注意力输出与源节点自身特征（残差连接，保留自身信息）
    # 通过 MergeLayer 融合注意力结果和节点自身特征，既保留节点原始信息，又融入邻居和时间的影响
