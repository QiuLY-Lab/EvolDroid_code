...
class RNNMemoryUpdater(SequenceMemoryUpdater):
  def __init__(self, memory, message_dimension, memory_dimension, device):
    ...
    self.memory_updater = nn.RNNCell(input_size=message_dimension,
                                     hidden_size=memory_dimension)
    # 创建一个 RNN 单元（nn.RNNCell）作为记忆更新器
    # input_size=message_dimension 指定输入（消息）的维度
    # hidden_size=memory_dimension 指定隐藏层（记忆）的维度
    # 这个 RNN 单元将用于根据输入消息更新记忆状态   更新记忆！！！
    # 举例
    # # 新消息（比如用户说的一句话）
    # new_message = torch.randn(1, 100)  # 形状: [1, 100]，100维的消息向量
    #
    # # 用RNN单元更新记忆
    # updated_memory = self.memory_updater(new_message, previous_memory)
    # # updated_memory 是新的记忆状态，形状: [1, 200]
